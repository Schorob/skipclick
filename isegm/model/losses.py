...
class BalancedBCE(nn.Module):
    ...
    def forward(self, pred, label):
        """
        Arguments:
            pred (torch.Tensor): A predicted logit mask in in 1xHxW format
            label (np.array): [B, 1, H, W] ground truth mask; binarized to 0/1
            ignore_index : A scalar integer, which has the value that is to be found at
                pixel positions, that are not to be considered when computing the loss.
        """

        label = torch.tensor(label)
        label = F.interpolate(label.float(), size=(pred.shape[-2], pred.shape[-1]))
        fg_loss = self.criterion(pred, label) * (label == 1).int().float()
        bg_loss = self.criterion(pred, label) * (label == 0).int().float()
