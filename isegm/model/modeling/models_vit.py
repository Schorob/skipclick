...
class VisionTransformer(nn.Module):
    ...
    def forward_extract(self, x):
        ...
        x = self.pos_drop(x + self.interpolated_pos_embed(h, w))
        x = self.blocks(x)

    def interpolated_pos_embed(self, h, w):
        h0, w0 = h // self.patch_size[0], w // self.patch_size[1]

        if h0 == self.grid_size[0] and w0 == self.grid_size[1]:
            return self.pos_embed
        else:
            pos_emb_2d = self.pos_embed[:, 1:].reshape(1, self.grid_size[0], self.grid_size[1], self.embed_dim).permute(0, 3, 1, 2)
            pos_emb_2d_scaled = F.interpolate(
                pos_emb_2d, size=(h0, w0), mode='bicubic'
            )
            pos_emb_scaled = pos_emb_2d_scaled.permute(0, 2, 3, 1).reshape(1, h0 * w0, self.embed_dim)
            pos_emb_scaled = torch.cat([self.pos_embed[:, :1], pos_emb_scaled], dim=1)
            return pos_emb_scaled
...
class EncoderStack(nn.Module):
    """ Encoder stack; no patch embedding
    """
    def __init__(self, img_size=(448,448), patch_size=(14, 14), embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True, pos_drop_rate=0., attn_drop_rate=0.,
                 proj_drop_rate=0., norm_layer=None, act_layer=None, primitive_stack = True):
        super().__init__()
        self.num_features = self.embed_dim = embed_dim
        self.img_size = img_size
        self.patch_size = patch_size
        self.primitive_stack = primitive_stack
        self.grid_size = (img_size[0] // patch_size[0], img_size[1] // patch_size[1])
        num_patches = self.grid_size[0] * self.grid_size[1]

        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim)) # learnable positional embedding
        self.pos_drop = nn.Dropout(p=pos_drop_rate)
    ...
    def patchify(self, x):
        ...
        win_h_grid = 224 // self.patch_size[0]
        win_w_grid = 224 // self.patch_size[1]
        win_h, win_w = grid_h // win_h_grid, grid_w // win_w_grid
        x = x.view(B, win_h, grid_h // win_h, win_w, grid_w // win_w, C)
        x_patchified = x.permute((0, 1, 3, 2, 4, 5)).contiguous()
        x_patchified = x_patchified.view(B * win_h * win_w, grid_h * grid_w // (win_h * win_w), C)
    ...
    def interpolated_pos_embed(self, x, h, w):
        h0, w0 = h // self.patch_size[0], w // self.patch_size[1]

        if h0 == self.grid_size[0] and w0 == self.grid_size[1]:
            return self.pos_embed
        else:
            pos_emb_2d = self.pos_embed.reshape(1, self.grid_size[0], self.grid_size[1], self.embed_dim).permute(0, 3, 1, 2)
            pos_emb_2d_scaled = F.interpolate(
                pos_emb_2d, size=(h0, w0), mode='bicubic'
            )
            pos_emb_scaled = pos_emb_2d_scaled.permute(0, 2, 3, 1).reshape(1, h0 * w0, self.embed_dim)
    ...
    def forward(self, x, h, w, additional_features=None, shuffle=False):
        if additional_features is not None:
            x += additional_features


        x = self.pos_drop(x + self.interpolated_pos_embed(x, h, w))
        num_blocks = len(self.blocks)
        # assert num_blocks % 4 == 0 # <- ???

        if self.primitive_stack:
...
class MAEExtractor(nn.Module):
    def __init__(self, extractor_type, backbone_weight_path, extraction_blocks=[2, 5, 8, 11]):
        super(MAEExtractor, self).__init__()
        self.extractor_type = extractor_type
        self.extraction_blocks = extraction_blocks
        self.feature_map_dict = dict()

        if self.extractor_type == "mae_vitb16":
            self.mae = vit_base_patch16()

        self.mae.fc_norm = None
        self.mae.head = None
        # Load the state dict
        sd = torch.load(backbone_weight_path)
        del sd["model"]["norm.weight"]
        del sd["model"]["norm.bias"]
        self.mae.load_state_dict(sd["model"])
    ...
    def forward(self, input):
        ...
        for block_num in self.extraction_blocks:
            ...
            out_features.append(features[:, 1:])
...
if __name__ == "__main__":
    test = "mae"
    if test == "mae":
        model = MAEExtractor("mae_vitb16", "/data/raid_ssd/schoerob/weights/mae/mae_pretrain_vit_base.pth")
        rand_img = torch.randn((4, 3, 448, 448))
        output = model(rand_img)
        for o in output:
            print("o.shape: ", o.shape)

    elif test == "encoder_stack":
        f_rand = torch.randn((4, 32 * 32, 768))
        test_stack = EncoderStack()
        output = test_stack(f_rand)
        print(output.shape)
