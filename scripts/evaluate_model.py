...
def parse_args():
    ...
    parser.add_argument('--datasets', type=str, default='GrabCut,Berkeley,DAVIS,SBD,BraTS,OAIZIB,HQSeg',
    ...
    parser.add_argument("--full-sam-eval", action='store_true', default=False)
    parser.add_argument("--wseseg-eval", action='store_true', default=False)
    ...
    parser.add_argument('--suppress-zoom', action='store_true', default=False)
    parser.add_argument('--eval-fvit', action='store_true', default=False)
    parser.add_argument('--eval-cmx', action='store_true', default=False)
    ...
    parser.add_argument('--visualize-results', action='store_true', default=False)
    parser.add_argument('--visualization-path', type=str, default='',
                        help='The path to the visualization directory. Will be created if it does not exist.')
    ...
    if args.visualize_results:
        assert args.visualization_path != ''
        Path(args.visualization_path).mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    if args.full_sam_eval:
        args.datasets = "PPDLS,BBBC038,DOORS,TimberSeg,NDD20,LVIS,STREETS,ZerowasteF,IShape,ADE20kSubsampled"
    if args.wseseg_eval:
        args.datasets = "WSESeg_flickr_bobsleighs,WSESeg_flickr_curling_stone,WSESeg_flickr_ski_helmets,WSESeg_flickr_snow_kites,WSESeg_yt_sports,WSESeg_flickr_curling_brooms,WSESeg_flickr_ski_goggles,WSESeg_flickr_ski_misc,WSESeg_flickr_slalom_obstacles,WSESeg_flickr_snowboards"
    ...
    for dataset_name in args.datasets.split(','):
        ...
        for checkpoint_path in checkpoints_list:
            ...
            print("Computing number of parameters... ", end=" ")
            num_params = 0
            for param in model.parameters():
                num_params += param.numel()
            print("Done: ", num_params)

            predictor_params, zoomin_params = get_predictor_and_zoomin_params(args, dataset_name, eval_ritm=args.eval_ritm)
            if args.suppress_zoom:
                zoomin_params['suppress'] = True
            else:
                zoomin_params['suppress'] = False

            # For SimpleClick models, we usually need to interpolate the positional embedding
            if (not args.eval_ritm) and (not args.eval_cmx) and (not args.eval_fvit):
    ...
    if apply_zoom_in and eval_ritm:
        if args.eval_mode == 'cvpr':
            if dataset_name == 'DAVIS':
                target_size = 600
            elif dataset_name == 'HQSeg':
                target_size = 1024
            elif dataset_name[:6] == 'WSESeg' or dataset_name == "SHSeg":
                target_size = 1024
            else:
                target_size = 400
            zoom_in_params = {
